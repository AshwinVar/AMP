...
MUTATIONS = [
    ...
    # The list pages through paging.page (ADR-0036); the clamp lives in
    # paging.clamp, so two of these bend the shared helper and two the call.
    ("the Approvals list ignores its offset (no way past page one)", "agent_routes.py",
     ...
     "                       AGENT_ACTIONS_PAGE, limit, 0, max_page=AGENT_ACTIONS_PAGE)"),
    ("the Approvals list page is uncapped", "agent_routes.py",
     "                       AGENT_ACTIONS_PAGE, limit, offset, max_page=AGENT_ACTIONS_PAGE)",
     "                       AGENT_ACTIONS_PAGE, limit, offset, max_page=10**9)"),
    ("a negative offset reaches the database", "paging.py",
     "    return max(1, min(limit, max_page)), max(0, _int(offset, 0))",
     "    return max(1, min(limit, max_page)), _int(offset, 0)"),
    ("rows sharing a timestamp have no stable order to page by", "agent_routes.py",
     ...
     "                       q.order_by(models.AgentAction.created_at.desc()),"),
