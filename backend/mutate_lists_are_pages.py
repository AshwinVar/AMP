...
MUTATIONS = [
    ...
    ("the cache ignores the bound values: one customer's total is served for another", P,
     "    params = tuple(sorted((k, repr(v)) for k, v in compiled.params.items()))",
     "    params = ()"),
    ("writes never forget the totals: a click's own refresh reads the stale count", P,
     "install_invalidation()" + chr(10),
     "pass" + chr(10)),
    ("a bulk UPDATE (read-all's path) is not seen by the cache", P,
     "    if state.is_update or state.is_delete:",
     "    if False:"),
    ("a write to one table forgets every table's totals (needless recounts)", P,
     "        if entry[2] & tables:",
     "        if True:"),
