...
- **Rule‑based Q&A** (`ai/assistant.py`, `POST /copilot/ask` with body `{question, thread?}` — `thread` is the caller's own prior turns, each question and the calls AMP ran, so "and its downtime?" follows the machine the conversation named; nothing is stored and every turn is authorized afresh, ADR-0035): a keyword router sends the question to the right pillar read‑model — OEE, cost of losses, delivery, downtime, quality, maintenance, compliance, inventory, machines (ask one **by name**, e.g. "how is SMT‑Reflow‑01 doing?"), production, WIP, shifts, and **week‑on‑week trends** — and phrases its live numbers into a sentence, with a one‑tap drill‑in to the view that owns the detail. "help" lists what it can answer.
...
- **Optional LLM layer** (`ai_copilot.py`): connecting an API key additionally enables free‑form conversational answers and an AI‑written management report, grounded in the same live factory data. It stays off (and free) until connected, and **degrades gracefully** — any LLM failure (no credits, rate limit, outage, a retired model) falls back to the rule‑based answer, so a customer never sees a raw API error. Two providers, chosen by environment only:
  - **Anthropic** (`AI_PROVIDER=anthropic` + `ANTHROPIC_API_KEY`, default model `claude-haiku-4-5`) — paid, with commercial data terms; the choice for real clients. Either hosted provider is used for a company only after that company's Admin turns on "Send Copilot questions and evidence to a hosted AI model" under Agent Activity → AI consent (ADR-0037); until then the Copilot answers from AMP's own engine and says so. The consent names the provider it was given for and lapses if AMP is switched to the other one (ADR-0038).
  ...
  - **Self‑hosted** (`AI_PROVIDER=local` + `AMP_LLM_BASE_URL` + `AMP_LLM_MODEL`, any OpenAI‑compatible runtime — Ollama, llama.cpp, vLLM; ADR‑0023) — factory data never leaves AMP's own infrastructure, and **no hosted key is required**. Its wording is used only once that exact model has passed the Copilot evaluation and its record is committed to `ai/adopted_models.json` (ADR‑0034: the first real candidate, `qwen3:8b`, passed with 0 unauthorized disclosures and routing at or above AMP's own engine).

  An explicit `AI_PROVIDER` always decides; unset, auto‑detection walks Anthropic, then Gemini, then the self‑hosted model — the self‑hosted one deliberately last, because configuring it is not the same as trusting it (its wording needs an adoption record). `GET /ai/status` reports the active provider, the resolved model, and (founder‑only) the last LLM error. **Production currently runs the Gemini free tier**; switching to Anthropic for a real client is one variable plus credits.
...
- **Money** — `cost`: downtime + scrap in good units not made, valued in £ only at the tenant's own unit value (ADR‑0010), by loss type / line / machine (biggest first), a daily trend, and recorded costs by type.
...
- **Technical:** `register(app)` adds `/ai/status`, `/ai/ask`, `/ai/report`. Both answering endpoints go through `ai/orchestrator.py`: the model (when one is configured and, for the self‑hosted kind, adopted) selects typed AMP tools, `run_tool` authorizes and runs them, the model words the evidence, and `ai/grounding.py` refuses any wording whose numbers, names, citations or links are not in that evidence — a refused wording is replaced by AMP's own sentence and labelled `source: "rules"`. Since ADR‑0034 `/ai/report` is the Daily Brief tool through that same path, not a raw data snapshot handed to the model. Providers: `_ask_claude` (Anthropic Messages API), `_ask_gemini` (generateContent with model self‑discovery on 404/429) and `LocalOpenAIProvider` (OpenAI‑compatible `/v1/chat/completions` with native tool calling) — each over stdlib `urllib`, no SDK, key in a header not the URL. Every answer writes one structured `amp.copilot` log line (engine, model, tools and states, gate verdict, latency, token counts — never the question, answer or tenant).
...
- Set secrets as **environment variables** on Railway (`DATABASE_URL`, `SECRET_KEY`, `ALLOWED_ORIGINS`, optional language‑model settings `AI_PROVIDER` + `ANTHROPIC_API_KEY` / `GEMINI_API_KEY`, or the self‑hosted `AMP_LLM_BASE_URL` + `AMP_LLM_MODEL` (+ `AMP_LLM_PLAN_TOKENS`, `AMP_LLM_TIMEOUT`) — none of which is required, optional `SIM_TENANTS`, `SENTRY_DSN`, `GMATS_ADMIN_PASSWORD`) — never in code. Railway has no GPU, so a self‑hosted model in production is an infrastructure decision the founder has not yet made; locally it runs on loopback.
...
**Every list is a page, and says so (ADR-0036).** A `GET` that lists a table which grows with time returns its newest rows, newest first — a default page the size of the endpoint's old cap (work orders 200, purchase orders 500, notifications 500, …) — and carries the caller's **whole** count in the `X-Total-Count` response header, so a page can be told from a complete list. `?limit=` (at most 2,000, or the endpoint's own smaller ceiling) and `?offset=` page through the rest. `/machines` is the deliberate exception: it is never capped (ADR-0010).
...
| `GET`/`POST`/`PATCH` | `/notifications[/{id}]` | List (a page of the newest 500; `?unread=true` keeps only notifications not yet Read, so with `limit=1` the `X-Total-Count` header is the tenant's whole unread count) / create / mark‑read notifications. |
