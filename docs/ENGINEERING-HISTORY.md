...
- **`backend/main.py` monolith.** *(Resolved, ADR-0009.)* Was 4,274 lines / 192 endpoints — the standing structural debt. **Seventeen** domains have since been peeled into `register(app)` route modules, each preserving the route-count invariant and shipping a registration-guard test: `read_model_routes` (#143), `agent_routes` (#146), `saas_routes` (#147), `costing_routes`, `machines_routes`, `orders_routes`, `factory_ops_routes` (#153), the core-CRUD + reporting wave — `work_orders_routes` (#154), `inventory_routes` (#155), `quality_routes` (#156), `production_planning_routes` (#157), `industrial_iot_routes` (#158), `operator_routes` (#159), `users_routes` (#160), `reports_routes` (#161) — and the intelligence cluster: `analytics_routes` (#165, 27 endpoints — the largest peel-off, after relocating its compute helpers to `analytics_engine` in #164) and `recommendations_routes` (#167). Event-publishing handlers kept their `event_bus.publish(...)` on the request session (guard-tested), so `ProductionCompleted` / `InventoryLow` / `QualityInspectionFailed` still commit atomically after the move. Now **706 lines / 12 endpoints** — the irreducible core (auth/bootstrap, sim-stateful `/platform/status` + `/ops-trends`, `/bom`, three intelligence stragglers). The guard-test discipline paid out twice: the survey for the first peel-off found and removed a dead shadowed duplicate `/health` (#142), and extracting the audit domain surfaced a second shadowed duplicate — a dead `GET /audit-logs` behind `platform_routes`' — removed and consolidated in #166. With the domains separated, all 22 route modules were then migrated from the `register(app)` pattern to FastAPI `APIRouter`s (#169–#171, three mechanical batches) — handlers now live at module level and `main` wires each with `app.include_router(...)`; the route-count invariant held at 237 throughout.
...
| Navigation views | 42 |
| Largest file | `backend/factory_simulator.py` — 1,010 lines. `backend/main.py`, once the largest at 4,274, is down to 706 after the ADR-0009 route-module extraction (still the most-touched, 92 commits) |
...
AMP today is a modular monolith with an event backbone, an AI platform of 31 read-models and 5 supervised agents, complete multi-tenant commercial machinery, and 59 test suites — built by one person in 45 days, deployed continuously, and documented by nine architecture decision records that explain not just what was chosen but what was rejected and why. The `main.py` monolith that was the standing structural debt at the time of first writing has since been fully peeled into seventeen domain route-modules (ADR-0009), taking it from 4,274 lines to 706 — the irreducible core — with the route-count invariant held at every step, and catching two dead shadowed-duplicate routes along the way.
