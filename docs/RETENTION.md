...
| `report_requests` | `created_at` | **180 days** | A request log, not an archive. Nothing generates a file from a request (each row's status is "Logged"); the CSV exports are generated on demand and never stored. |
