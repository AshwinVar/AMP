...
| 13 | Closed-loop actions, with the result measured | BUILT · **now reachable from a conversation** (ADR-0039) | The loop had no conversational entrance: all 26 typed tools were reads, so "create a maintenance task" fell through to the briefing tool and answered with a plant summary. A tool now DRAFTS one (`ai/tools/actions.py`, writing nothing), a person raises it through `POST /agent-actions/propose` — which takes only `kind` and `machine_id` and re-derives everything else from the machine — and the existing gate executes it. A model is on none of the three steps, and `agent="copilot"` can never auto-approve. · ADR-0029: migration `0011` and `action_outcomes`; an approval freezes the metric it was meant to move, the window is left to elapse before anything is judged, the reading is frozen once, and every change is labelled CORRELATION with the caveat attached. `GET /action-outcomes`, the `get_action_outcomes` tool and a card on the agent view |
