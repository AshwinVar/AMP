...
6. **Where the ~19 ms above every handler goes.** `dashboard_perf` (handler only, PostgreSQL) vs `loadtest` (whole request) at 10 machines: handlers are 1.7–9.3 ms, requests are 21.5–73.5 ms. Serialisation is NOT it — `jsonable_encoder` + `json.dumps` measured 0.0–2.6 ms even on the 47 KB payload. Auth, session setup and middleware are unexamined. This is a constant tax on all 49 endpoints per refresh, so it is worth more than any single endpoint.
