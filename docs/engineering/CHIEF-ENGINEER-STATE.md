...
**The figures angle's second item, this PR: shift attainment pools one
window.** The formula was unified in the no-target fix; the span never was —
...
The quality fail rate (7-day `ai/quality`, lifetime `/analytics/quality` and
the command centre) is the same shape and the next item.
