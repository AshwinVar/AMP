...
## ⚑ BUILD-FIRST MODE — active since 2026-09-22

The founder changed engineering strategy. Read this before choosing what to do
next; it overrides the working rhythm the rest of this file describes.

**Build in vertical slices, test at checkpoints, ship coherent feature PRs.**
Roughly 70% building / 20% testing and hardening / 10% documentation. Do not run
the whole backend or frontend suite after every small edit, do not write a
mutation harness for every function, do not open a PR per correction, and do not
interrupt development to re-sync every training document. Targeted checks while
building; comprehensive testing at the slice checkpoint.

**The exceptions, where strong testing is still immediate and mandatory:**
authentication, authorization, tenant isolation, OEM consent, secret handling,
database migrations, destructive writes, financial transactions, **AI
write/action authorization**, production deployment safety. Mutation testing
...
a production outage, data corruption, an auth failure.

The build order the founder named: Owner Intelligence · **Native AI** ·
Action Intelligence · Machine Intelligence · Customer Onboarding, with the
Native Copilot called out as the priority and a deterministic sales-demo factory
as the proof surface. Do not stop after a PR, a slice or a checkpoint.
...
**Master SHA:** `c05afaa` (#708).
**Production SHA:** `c05afaa`, verified live, not assumed: `/health` returned
`version: c05afaa` on 2026-09-22 10:27 UTC, minutes after the merge; `/readiness`
200, `/daily-brief` refuses an unauthenticated call with 401, the frontend 200.
Before it, #707 `21278b8` at 06:43 UTC and #706 `0830962` at 05:13 UTC, verified
the same way. Before it, #705 `e669146` at
04:31 UTC and #704 `e8cffb1` at 03:44 UTC, verified the same way. Before it, #703 `5ac43fa` at 03:15 UTC and #702 `2fa89a3` at 02:26 UTC
(where `POST /agent-actions/propose` was checked the same way); `/readiness` 200; **`POST /agent-actions/propose` — the
new write on the Copilot's action path — refuses an unauthenticated call with
401**, as does `/copilot/ask`; the frontend answers 200. Before it, #701 `9e3f122`
was verified the same way at 2026-09-22 00:38 UTC;
at the #701 merge itself (2026-09-21 23:49 UTC) `/readiness` answered 200 and
`/downtime-summary`, `/downtime-trend`, `/quality-trend` and `/analytics/summary`
each refused an unauthenticated call with 401, and the frontend answered 200. The trail behind it, each verified the same way at its own merge — #700 `1f55eab` (2026-09-21 22:29 UTC): `/readiness` 200; `/analytics/final-executive-summary`, `/analytics/maintenance`, `/analytics/operator-terminal`, `/mission-control/pulse` and `/agent-actions/impact` refuse an unauthenticated call with 401; the frontend answers 200 (#699 `b7a027e` at 21:13 — `/analytics/quality`, `/quality-summary` and `/quality-trend` 401 — #698 `f3c6b55` at 20:13 — `/analytics/summary` and `/shift-summary` 401 — #697 `bea0ee9` at 16:26 — `/mission-control/pulse` and `/machine-health` 401 — #695 `6c51b13` at 15:56 — `/recovery-summary` 401 — #696 `21de3a7` at 15:51 — `/risk-radar` 401 — #694 `552ea46` at 15:29 — `/machines` 401 — #693 `61f6398` at 15:05 — `/oem/fleet?limit=1` 401 and the portal page `/oem` 200 — #692 `e4ef9f5` at 14:05, #691 `ec836a3` at 11:51 — `/issue-slips` refuses an unauthenticated call with 401 — #690 `8eabba9` at 11:42 — `/industrial/mappings` and `/oem/notifications` refuse an unauthenticated call with 401 and `POST /register` refuses a body carrying a `role` with 422 — #689 `8459ad3` at 11:02, #688 `c7c674d` at 10:35 — `schema: ok` there means migration `0012_consent_scope` ran on the production database — #687 `854a634` at 09:55, #686 `82d1c4d` at 09:45, #685 `db2f074` at 09:47, #684 `4149829` at 07:00 and #683 `ca4f257` at 06:49 were verified the same way); `/ai/status`, `/ai-consent` and `POST /industrial/devices` refuse an unauthenticated call with 401. The Monday mutation fleet, dispatched by hand after #685 (the standing rule for AI-consent code), ran green on master at `db2f074`: every harness caught every mutation (run #3, 26 min). The frontend (`https://flow-mes.vercel.app`)
...
stock and sales writes were offered to an Operator (two of them to everyone)
and answered 403. The server decides, unchanged; a control it would refuse is
not offered, and a note says who can (`lib/roles.ts`, `components/RoleNote.tsx`,
`lib/agent-actions.canDecideProposals`; `lib/dashboard-role-gates.test.ts` is a
textual guard for the four inline dashboard controls, which have no render
test). The handbook's roles matrix said an Operator may change a machine's
status — the route is Supervisor+ — and now lists what each role is offered.
Figures: the OEE family is hardened; ten figures beside it are not, ranked in
the table. Clock: one live defect, fixed in **#693 `61f6398`** — the OEM portal's fleet headline
...
stranded `lib/oem.ts` anchors re-pointed). The swallowed-failures angle, re-run
against the subscribers, jobs and frontend write paths: the backend's jobs and
subscribers are clean (every swallow is documented and the failure is the
answer); three frontend write paths were not — a failed "Raise an escalation"
called the same callback as a success and walked the owner to the Escalation
...
`test_predictive_engine.py` added to its suites). Merged as **#697 `bea0ee9`**.

**The figures angle's second item: shift attainment pools one window.** The formula was unified in the no-target fix; the span never was —
the shift card pooled the week, `/analytics/summary` and `/analytics/management`
every shift ever recorded, `/analytics/executive-oee` the most recent 50 rows.
`shift_contract.pooled_attainment(db, tenant, window)` (the canonical
`OeeWindow`, tenant-explicit SQL, `None` without a target, `measured` /
`window` / `days` beside the number) is read by all four; `rows_in` feeds the
breakdowns from the same window so a chart sums to its headline; the legacy
integers keep their 0 with a `*_measured` flag; the Achievement tile and the
shift card name the window and show a dash when nothing was planned;
`daily-summary.txt` says the window. `test_shift_rollups_one_window.py` is the
shift half of `test_oee_rollups_one_window.py`; `mutate_shift_window.py` (10/10).
CI caught one thing the local run could not: `test_lists_are_pages.py` listed
`get_executive_oee` as a capped, unpaged GET, and replacing its `.limit(50)`
with the window made that allowlist entry stale — the guard fails the build for
an entry that no longer names a real capped read, which is exactly what it is
for. Merged as **#698 `f3c6b55`**.

**The figures angle's third item, this PR: the quality fail rate pools one
window, and says nothing when it measured nothing.** Three spans under one
label: `ai/quality` on seven calendar dates with no upper bound, and both
`/analytics/quality` and `/analytics/factory-command-center` on the entire
inspection register — while `ai/twin._machine_quality` had been moved onto the
canonical window by #590, so the machine cockpit disagreed with the plant by
construction. On a plant that ran badly last quarter and cleanly this week the
Quality view and the digital twin read 21% and the intel card read 1%; all
three were arithmetically right and none of them said which week it meant.
`quality_contract.plant_quality(db, tenant, window)` is now the one figure —
tenant-explicit SQL like `oee_contract.plant_oee`, `rows_in` on the ADR-0002
hook for the breakdowns — and the Pareto and per-machine lists under each
headline are bounded by the same window, so a chart sums to the number above
it. The trend's halves are `window` and `prior_window(window)` instead of
calendar-day ages, so the level on the trend card IS the level on the summary
card beside it rather than a second computation that agreed by luck.
`_window_span` moved from `ai/twin.py` into `oee_contract.window_span`, because
the quality trend needed the same span and a third copy of that rule is how
these panels drifted apart in the first place.

The second half is the same "0 is not nothing" line as #697 and #698: `round(x
/ y * 100) if y else 0` was in five places, and 0% is the BEST value on a
fail-rate scale, so a plant that stopped inspecting published a perfect
quality record. `rate()` returns `None`, `measured` says which it was, and
every screen prints a dash with the reason. The consumers had to come along or
crash: the briefing's `quality["fail_rate"] >= FAIL_RATE_ALERT` is a TypeError
against `None`, and `ev.Fact` refuses a missing value that is not labelled
UNKNOWN, so the Copilot's quality tool now returns PARTIAL DATA with an UNKNOWN
rate fact instead. `mutate_quality_window.py` caught 15 of 18 on its first run;
all three survivors were real test gaps, not harmless mutations — the cockpit
drawer was never asserted on an unmeasured machine, the briefing fixture had no
production record so `build_briefing` took its "no production data yet" early
return and never reached the quality alert at all, and nothing pinned a window
ending at exactly midnight, the only instant where `window_span`'s exclusive
upper end changes the answer. 18/18 after covering them. Merged as **#699
`b7a027e`**.

**The figures angle's fourth item: six tiles that said `0` when they
measured nothing.** A read-only agent verified all six recorded claims against
master before a line was written, and all six held. The rule is the one #697,
#698 and #699 each applied to their own figure, and it is worth stating as a
rule because the zero is never neutral — it is a real reading, and which lie it
tells depends on the scale. "Achievement 0%" said the book failed to deliver
everything it promised. "Quality 0%" said every part the crew made was scrap.
"Avg Repair 0m" was a perfect maintenance record. "Autonomy 0%" said every
agent decision needed a human. "Avg Utilization 0%" was an idle plant. Each now
returns `None` with a `*_measured` flag, and each tile prints a dash.

This PR also pins the line the rule actually draws, which the earlier three left
implicit: a SUM or a COUNT over an empty set really IS zero. Money not spent is
zero money, so `total_cost` stays a number — and `mutate_not_measured.py`
mutates in that direction too, because refusing to state a figure we do have is
the same defect reversed.

Two of the six were also on the wrong span. The command header's Autonomy tile
showed the LIFETIME auto-approval rate under a caption reading "N actions / 7d"
— the only window named on the tile, attached to the denominator of an all-time
rate, so a fleet that auto-approved everything last year and nothing since read
"Autonomy 100% · 12 actions / 7d". `ai/impact` also cut its own `utcnow() - 7d`
with no upper bound, so a future-dated action inflated that very count. It now
uses the canonical `OeeWindow`, publishes the week's own rate beside the
lifetime one, and the header shows the week's. And
`/analytics/final-executive-summary` held a FOURTH lifetime copy of `passed /
inspected` — the figure #699 had just unified across three surfaces — so an
executive page could contradict the Quality view beside it; it reads
`quality_contract` now and names the window.

`avg_utilization` gained the thing it never had: coverage. It averages the
machines with a reading and now says how many that was out of the plant, the
same survivorship problem `oee_contract.coverage` exists for one figure to its
left, and the daily summary prints "from N of M machines" exactly as the plant
OEE line does. Merged as **#700 `1f55eab`**.

**The window thread's last user-facing card, this PR: downtime.** The module's
own `_half_of` docstring already said it was next, and the defect turned out to
be bigger than the trend. `ai/downtime` cut the last seven CALENDAR dates and
filtered the rest in Python, while `/analytics/summary` and
`/analytics/executive-oee` passed the canonical window to
`analytics_engine.downtime_aggregates` — for the same plant, on the same
screen. Measured before the fix: a six-hour breakdown late on the day the
window opens read 0 events on the Downtime card and 360 minutes on the tile
beside it, and a stoppage dated later today read the other way round, so the
card counted a stoppage that had not happened yet (the future-dated defect #590
found on the machine cockpit and #696 found in the Risk Radar).

All three builders — summary, reason drill-down and trend — now take the
canonical `OeeWindow`, bounded at both ends in SQL rather than by a Python
date-set. The series spans `window_span` with the oldest bucket flagged
`partial`; the trend's halves are `window` and `prior_window(window)`, each
queried on its own bounds. `_norm_reason` is now
`analytics_engine.normalize_downtime_reason` — the engine's own docstring had
required these two copies to agree, and they are one function now.
`test_downtime_one_window.py` pins the reconciliation against
`downtime_aggregates` directly, so the card and the tile cannot drift again.
...
|---|---|---|
| **2026-09-22, "AI Predictive Intelligence" suggested things and offered no way to act on any of them.** An `AIRecommendation`'s only futures were Acknowledged and Closed: it created no task, entered no approval gate and tracked no outcome, while the `AgentAction` path beside it did all three (ADR-0005, ADR-0029). Two "AI suggests" surfaces behaving differently is confusing on its own; the weaker one carrying the stronger name is worse | P2 | fixed: every recommendation now carries `action` (a sentence) and, where AMP can carry it out, `propose` — the ADR-0039 draft, raised through the same `POST /agent-actions/propose` and executed by the same gate. ONE kind of five qualifies: a maintenance task on a machine is the only entry in `ev.PROPOSABLE_KINDS`, so ordering stock and rebalancing a schedule get a sentence, not a button. Computed on READ, not stored — no migration, and a recommendation raised last week is judged against what AMP can do today. A maintenance recommendation naming NO machine offers nothing, because the write route would have no id to resolve and a button that 400s is worse than no button. The component had **no test at all** before; it has six. `test_recommendations_routes.py` +1, `AIInsightsSection.test.tsx` 6/6; backend sweep 360 files green; all FOUR frontend gates run this time (97 files / 871 tests, tsc, eslint 132, **Playwright 23 passed**); **this PR** |
| **2026-09-22, the Daily Brief showed an owner nothing but its own disclaimer.** Every section rendered collapsed and `open` was a single key, so opening one closed another. Measured on the demo plant: **32 lines of content across seven sections, and a morning brief showed none of it** — seven closed headers and the amber "what AMP could not see" box. That box is right to be always visible (it is what stops the rest being read as the whole picture); the defect was that it was the ONLY thing on screen | P2 | fixed: `position`, `problems` and `actions` — the three that carry the brief's answer, 12 lines — open on load; the supporting four stay one tap away; `open` is a Set, so reading the shifts no longer costs you the problems. The existing test asserted the collapsed default, i.e. it encoded the defect: replaced with four that pin the new intent, including that an opened section still closes and that opening one does not close another. `DailyBriefSection.test.tsx` 10/10; frontend 96 files / 865 tests; **#708 `c05afaa`** |
| **2026-09-22, the owner's home screen had forgotten machine health.** The 0–100 score, its band and the eleven-rule explanation all existed on `/machine-health` (ADR-0027) and the Command Centre counted `running/total` and nothing else, so somebody who had seen the Machine Health page would ask why the main screen did not show it | P2 | fixed: `twin.fleet_health` — ONE definition, because `ai/pulse` already had the arithmetic inline and the card needed the same number; a second copy is how two screens start disagreeing about one plant. `ai/pulse` now calls it (its suite passed untouched, which is what makes the extraction behaviour-preserving) and the card shows the same average, with ADR-0027's rule intact: a machine with nothing recorded scores 100 by ABSENCE, only measured machines count, the coverage is stated when partial, and the average is `None` — never 0 — when nothing could be read. The tile renders nothing rather than throwing on a payload with no health block, because a rolling deploy leaves the new bundle talking to the old backend — the existing fixture proved that case was live by crashing nine tests before it was guarded. The card's query budget: 37 of 60. `test_command_centre.py` §8 (the card and the pulse report the same average; an unrecorded machine does not drag it up), `CommandCentreSection.test.tsx` +4; **#707 `21278b8`** |
| **2026-09-22, work orders reached NO owner surface.** Neither `ai/command_centre` nor `ai/risk_radar` read `models.WorkOrder` at all: the plan rows come from `ProductionPlan` and the delivery rows from `CustomerOrder`, so "which jobs are late?" — the first question a job shop asks — was answerable only by opening the Work Orders tab and reading a list. A customer order can be late BECAUSE the job is late, and the radar could see the first and not the second | P2 | fixed: `_work_order_problem` on the Command Centre (sized in the units still to make; UNSIZED rather than zero when no order carries a target, and the `undated` count is stated because an order with no planned end is not on time, it is unjudgeable) and `_work_order_risks` on the radar. The card's query budget is a recorded guard — 27 of 60 after the new read. The demo now plants a late work order that is the REASON its late customer order is late, so an owner clicking through finds the job rather than two unrelated problems sharing a customer. Two defects in my own new copy, both caught by reading the output: the action said "open 0 days and its date has gone" (both halves true, the sentence nonsense — `age_days` comes from `created_at`, which is legitimately today on an order raised against a back-dated plan), then "open 1 days"; the demo had the same inconsistency in its data and now stamps `created_at` from `planned_start`. `test_risk_radar.py` §8; backend sweep 360 files green, frontend 96 files / 858 tests; **#706 `0830962`** |
| **2026-09-22, the Risk Radar told you what was about to go wrong and never what to do about it.** Every risk carried WHY (its rule), IMPACT (where AMP can size it) and EVIDENCE (its facts) — and then ended in a deep link. Eight things about to go wrong with no remedy reads as an alarm panel, not an advisor, and it was the one AMP surface that did not end in a next step | P2 | fixed: every risk now carries `action`, a sentence naming the next step, and the one family AMP can actually carry out — a machine likely to stop — additionally carries `propose`, the ADR-0039 draft. Most risks have NO proposal and that is the point: ordering stock and chasing a customer are not AMP's to do, and a button for them would be a promise it cannot keep, so `_risk()` raises on any kind outside `ev.PROPOSABLE_KINDS`. The frontend write was extracted to ONE `ProposeActionButton.tsx` shared by the Copilot card and the radar, because two surfaces each carrying their own copy of the same POST is how one of them quietly starts sending a field the server re-derives anyway; `CopilotProposal`'s seven existing tests passed untouched, which is what makes the extraction behaviour-preserving. `test_risk_radar.py` §7 (every risk has an action; only machine risks are proposable; every proposed machine id is the tenant's own; a kind outside the closed set is refused at construction), `ProposeActionButton.test.tsx` 7/7, `RiskRadarSection.test.tsx` +3; frontend 96 files / 858 tests; **#705 `e669146`** |
| **2026-09-22, a factory without a gateway could not give AMP the one thing it measures.** `POST /production-records` has existed since the beginning and NO screen ever called it — every writer was a seeder, the simulator or the MQTT ingest. So an SME on day one, with no edge agent, had no way to record what a machine made, and the Command Centre said "no production recorded" forever: OEE, the product's headline number, was uncomputable by hand. Alongside it there was no first-run experience of any kind (0 hits for `wizard` across the repo): the empty states were honest ("No machines yet", "Fleet health not measured") and every one was a dead end, so the moment a new Admin most needed direction was the moment they got none | P1 | fixed: `ProductionEntryForm.tsx` (every role may record production, so it is not role-gated; it repeats the server's `good + rejected == total` rule for an instant answer and shows the server's own refusal verbatim, never a friendlier lie; "ideal cycle time" is explained where it is asked for, being the one term that is ours and not the customer's) and `SetupChecklist.tsx` (reads what the workspace already has, in the order the data depends on itself, and removes itself once done). It lists ONLY steps AMP can actually carry out — "create a site" is deliberately absent because `Machine.site` is in the schema and reachable from no form and no route, so a row for it would be an instruction nobody could follow. `TenantBranding` gained the `unit_value_gbp` the endpoint always returned. `ProductionEntryForm.test.tsx` 6/6, `SetupChecklist.test.tsx` 7/7; frontend 95 files / 848 tests, tsc clean, eslint 132 (baseline), build clean; **#704 `e8cffb1`** |
| **2026-09-22, the sales-demo factory could not be rehearsed, and had almost nothing to discover.** `reset_factory.py` imported `random` at line 18 and never seeded it, so production, downtime, reject rates, order due dates and work-order progress were redrawn on every reset — the numbers practised on were never the numbers on screen, and "the biggest measured loss" was a different sentence each time. Worse, of the SEVEN problem kinds `ai/command_centre` can discover it could plant exactly ONE (a machine down): it created no `InventoryItem` rows at all (so `_stock_problem` always returned None and the Copilot answered "no stock items are set up" to "what should I reorder?"), no `ProductionPlan` rows (so plan attainment was NOT CONFIGURED and the headline read "no production plan is set to compare against"), every `due_date` was `today + 3..20` (so no order could be late), both maintenance tasks were dated `today` against a strictly-`<` overdue predicate (so the overdue count was 0 on reset day), and no seeder ever set `unit_value_gbp` — so the demo's most persuasive surface, money, was off by default | P1 | fixed: `DEMO_SEED` seeds the RNG once in `rebuild_factory(db, seed=)`; `_seed_inventory` (upsert, because stock hangs off movements/POs/slips — `reseed_inventory.py`'s territory — and carries a unique `(tenant_code, item_code)`), `_seed_plans`, `_seed_shifts`, `_seed_unit_value`; one dominant downtime reason, one pinned quality outlier, one past-due order, one backdated maintenance task. All seven now discovered by AMP's own rules and priced: biggest loss £9,725, plan shortfall £4,125, quality £2,350, losses ≈£57,763 this week. `test_reset_factory.py` +3 (reproducible, every planted problem discoverable, the new seeders touch only DEFAULT); `docs/sales/FACTORY-DEMO-RUNBOOK.md` quotes the exact figures because they are now quotable; **#703 `5ac43fa`** |
| **2026-09-22, the Copilot could answer everything and do nothing** (the first slice under the founder's BUILD-FIRST directive). All 26 typed tools were reads; "create a maintenance task" matched no pillar, fell through `PILLAR_TOOL` to the briefing tool and came back as a plant summary — so AMP did not do the thing and did not say it had not. The fix does NOT register a write tool, because a model names tools and a tool that writes puts a model at the point a row is created (ADR-0022's chain). A tool DRAFTS; `POST /agent-actions/propose` writes, taking only `kind` and `machine_id` and re-deriving the priority, the task type and the wording from the machine itself; the ADR-0005 gate executes; ADR-0029 freezes the baseline. `agent="copilot"` can never auto-approve, whatever `AUTO_APPROVE_AGENTS` says. Mutation testing found the design error in my own first draft: I had forced every machine with an empty 30-day file to Medium priority and published its risk as UNKNOWN, which would have ranked a machine in breakdown RIGHT NOW below every machine that had merely been logged — three of the engine's eleven rules read current state, not history | P1 | fixed: ADR-0039, `ai/tools/actions.py`, `agent_routes.propose_agent_action`, `agents.propose_requested_task`, `CopilotProposal.tsx`; `test_copilot_actions.py` (14 sections), `mutate_copilot_actions.py` 23/23, `CopilotProposal.test.tsx` 7/7; **#702 `2fa89a3`**. Two more things this slice turned up: `assistant._help` ("what can you do?") listed only what AMP can TELL you, so the one thing it can now DO was undiscoverable — it now says so AND states the limit; and the 360-file backend sweep caught `test_mutation_anchors_apply.py` failing because moving `plan_rules`' condition onto three lines left an anchor in `mutate_copilot_follow_ups.py` matching nothing (a refactor had silently disabled an existing mutation — the guard working exactly as intended). Sweep: 360 files, all passing |
| **2026-09-21, five places that said something the code did not do** (found by a four-angle read-only sweep after the honesty line closed: accepted-but-inert inputs, UI copy promising an effect, doc guarantees vs code, a rule at N sites with one missing). **`/ai/report`'s rules fallback built the report for the TOKEN's tenant**: a founder previewing a company, when the orchestrator raised, got the company's rows priced at the founder's own unit value and the explicitly filtered sections empty — the defect `/ai/ask`'s docstring records as fixed, missed on this branch | P2 | fixed: `tenancy.request_tenant`, pinned by `test_ai_copilot_fallback.py` (the fallback names the previewed company) and a mutation in `mutate_external_model_consent.py` |
...
| **2026-09-21 (afternoon), a health score said "all 11 rules passed" for a machine with nothing recorded**: `calculate_predictive_risk` emits a row for every machine over `defaultdict(int)` counters, so a machine with no downtime row, no production record and no breakdown transition in the 30-day window read "0 min", "0 events", "0%" on the six history rules and scored 100 / Healthy on every surface; `NOT_SCORED`'s warning was unreachable, the twin's only `has_data` flag was OEE's, and `pulse.avg_health` averaged the phantom 100s (an empty fleet read "Fleet health 0") | P3 | fixed: the row carries `recorded_inputs` / `has_recorded_input` (presence taken before the reads); the explanation is PARTIAL DATA with those rules "not measured", a `health.rules_unmeasured` fact and a note that the score is an absence; the twin carries `health_measured`; the pulse averages the measured twins only, null when none, "N of M measured" in the headline; every screen and the Copilot's machine fact say it; `test_predictive_engine`, `test_machine_health_explained` §7b, `test_twin`, `test_pulse`, `FactoryPulse.test`, `MachineHealthSection.test`, `HealthExplanation.test`; `mutate_machine_health.py` +6 (31/31); **#697 `bea0ee9`** |
| **2026-09-21 (afternoon), shift attainment pooled three windows**: the shift card the week (its own rolling cutoff), `/analytics/summary` and `/analytics/management` every shift ever recorded (the latter three lines under a windowed production sum), `/analytics/executive-oee` the most recent 50 rows — a count, not a span; one plant read three attainments for the same shifts on the same day while the comment beside the lifetime sum claimed "the same basis as the shift read-model" | P3 | fixed: `shift_contract.pooled_attainment` (the canonical `OeeWindow`, tenant-explicit SQL, `None` without a target, `measured` / `window` / `days` beside it) read by all four surfaces, `rows_in` for the breakdowns; the legacy integers keep their 0 with `target_achievement_measured` / `production_achievement_measured`; the Achievement tile and the shift card name the window and show a dash when nothing was planned; `daily-summary.txt` says the window; `test_shift_rollups_one_window.py`, `mutate_shift_window.py` (10/10), and the stale `get_executive_oee` entry deleted from `test_lists_are_pages.py`'s allowlist (the route no longer caps by row count); **#698 `f3c6b55`** |
| **2026-09-21 (evening), the quality fail rate was three windows and a perfect score for a plant that inspected nothing**: `ai/quality` cut its own seven CALENDAR dates with no upper bound (the intel card, the snapshot, the briefing alert, the command centre, the copilot, the assistant); `/analytics/quality` pooled EVERY inspection ever recorded (the Quality view's Pass Rate / Fail Rate tiles and the six count tiles beside them); `/analytics/factory-command-center` pooled every inspection ever recorded (the twin's "Quality Fail"); `ai/twin._machine_quality` was already on the canonical window (#590), so the machine cockpit and the plant disagreed by construction — a plant bad last quarter and clean this week read 21% / 21% / 1%, and no screen said which week. Every one of them published `0%` — the BEST value on a fail-rate scale — when the window inspected no units | P3 | fixed: `quality_contract.plant_quality` (the canonical `OeeWindow`, tenant-explicit SQL, `rate()` returning `None` with `measured` / `window` / `days` beside it) read by all four surfaces, `rows_in` for the breakdowns and the drill-downs on the same bounds; the trend's halves are `window` + `prior_window` so its level IS the summary's; `window_span` moved into `oee_contract` (a series covers the dates its window touches, and not the day after one ending at midnight); the assistant, briefing, command centre, copilot and Copilot tool state "not measured" instead of reading a `None` aloud or crashing on `None >= 5`; every tile names its window and prints a dash; `test_quality_one_window.py`, `mutate_quality_window.py` (18/18), `QualityRates.test.tsx`; **#699 `b7a027e`** |
| **2026-09-21 (evening), six figures still published `0` for "nothing measured"**, and which lie that told depended on the scale: work-order and production-plan `achievement` (`0%` = the book failed to deliver), the operator terminal's `quality_rate` (`0%` = every part the crew made was scrap), `avg_repair_minutes` (`0m` = a perfect maintenance record), the agent fleet's `auto_rate` (`0%` = every decision needed a human) and `avg_utilization` (`0%` = an idle plant, with no coverage beside it). Two also disagreed about their span: the command header's Autonomy tile showed the LIFETIME rate under a caption reading "N actions / 7d" — the only window named on the tile, attached to the denominator of an all-time rate — and `ai/impact` cut its own `utcnow() - 7d` with NO upper bound, so a future-dated action inflated that very count; `/analytics/final-executive-summary` pooled a FOURTH lifetime copy of `passed / inspected`, the figure #699 had just unified across three surfaces | P3 | fixed: each rate returns `None` with a `*_measured` flag and every tile prints a dash; `avg_utilization` carries `utilization_machines` of `machines` and the daily summary says "from N of M machines"; the executive page reads `quality_contract`; `ai/impact` uses the canonical `OeeWindow` (bounded at both ends) and publishes the week's own `auto_rate`, which is what the command header now shows; the line the rule actually draws is pinned too — a SUM over an empty set IS zero, so `total_cost` stays a number; `test_rates_say_not_measured.py`, `mutate_not_measured.py`, `NotMeasuredTiles.test.tsx`; **#700 `1f55eab`** |
| **2026-09-21 (evening), the Downtime card and the tile beside it measured different weeks**: `ai/downtime` cut the last seven CALENDAR dates and filtered the rest in Python, while `/analytics/summary` and `/analytics/executive-oee` passed the canonical `OeeWindow` to `analytics_engine.downtime_aggregates` for the SAME plant. Measured: a six-hour breakdown late on the day the window opens read 0 events / 0 min on the card and 1 event / 360 min on the tile; a stoppage dated later today (a skewed gateway clock) read the other way round, so the card counted a stoppage that had not happened yet. The trend split its fortnight by calendar-day age, so a stop at 03:00 on the seam date landed in `prior` there and in `current` on the quality trend card printed beside it | P3 | fixed: all three builders take the canonical `OeeWindow`, bounded at BOTH ends; the series spans `window_span` with the oldest bucket `partial`; the trend's halves are `window` + `prior_window`, each queried on its own bounds; `_norm_reason` is now `analytics_engine.normalize_downtime_reason` (the engine's docstring already required the two to agree); the cards name the window from the payload; `test_downtime_one_window.py`, `mutate_downtime_window.py`; **#701 `9e3f122`** |
| **MOVED TO `BACKLOG-DEFERRED.md` on 2026-09-22** under the founder's build-first directive; kept here for the trail. One claim in it was WRONG and was corrected on the way out: `recommendations_routes`' fail rate does NOT drive a wrong "High" — its rows are already bounded (`created_at >= cutoff`, `:95`) and its `0` cannot fire a recommendation because the threshold is `>= 10` (`:171`). What is real is the missing UPPER bound. Recorded, not yet fixed, from the afternoon sweep. **Figures without a basis**: ten read-models still cut a private cutoff instead of `oee_contract.OeeWindow` and most have NO upper bound, so a future-dated row counts (`ai/production`, `ai/losses`, `ai/workforce`, `ai/roster`, `ai/trends`, `ai/coverage`, `ai/stock_health`, `ai/stock_accuracy`, `ai/supplier_performance`, `recommendations_routes`); `ai/trace._pct` and `recommendations_routes` still publish `0` for "nothing measured" (the two rate sites outside the contracts). **Swallowed failures** (re-run): the GMATS CSV import (`gmats_inventory_routes.gmats_import_csv`) has no per-row savepoint and commits inside the loop, unlike its enterprise sibling — latent: no unique index exists on `gmats_items` or `gmats_aliases`, so no row can fail at flush; `lib/dashboard-write-errors.test.ts` guards only `app/dashboard/page.tsx`, and the three write paths that swallowed a failure were all in `components/` (fixed above, #695). **Test isolation** (found while checking the single-process path before #699): `test_boot_migrations.py` fails when any `test_t*.py` suite has run before it in the same pytest process -- its reproduction drops a column to simulate the older schema and the User query then SUCCEEDS, so the fix it is meant to prove is never exercised. Reproduced on master at `f3c6b55` with `pytest -q test_t*.py test_boot_migrations.py`; passes standalone, and CI's coverage job (every suite, its own order) is green, so the guard is only silently inert under some orderings | P3 | recorded 2026-09-21 |
