...
MES reasons, accepted by both parties. That attribution is the differentiator.
Metering, pay-per-use and shared usage ledgers already exist (SteamChain,
PayperChain, Linxfour; Rockwell US10747201B2) and nothing here claims novelty for
them or builds one: no ledger, no hash chain, no usage billing. A
freedom-to-operate review is needed before commercial launch.

Creates eight empty tables: `service_contracts`,
`service_contract_term_versions`, `service_contract_machines`,
`contract_statements`, `contract_attribution_records`,
`contract_statement_acceptances`, `contract_disputes` and
`machine_telemetry_spans`. Each is skipped if boot's `create_all` already made
it. No existing table is altered and no row is written, so it cannot fail on
existing data and takes no lock on a populated table.

What the schema commits to, and where it is proved:

- **One statement per contract period** (`uq_contract_statement_period`) and
  **one acceptance per (statement, party, revision)**
  (`uq_statement_acceptance`). Revision is in the key on purpose: a party
  accepts revision 1 and, after a dispute is raised and withdrawn, must be able
  to accept revision 3 even though its bytes equal revision 1's.
  Validity itself is `canonical.acceptance_is_valid`, not a column.
- **No `Numeric` or `Float` column.** Money and percentages live as decimal
  text inside the JSON documents; SQLite stores `NUMERIC` as `REAL`.
- **The seven contract tables have no `tenant_code`.** They carry `oem_code`
  and `factory_tenant_code`, so `offboard_tenant.purge_tenant_data` cannot
  hard-delete an OEM's contract history when a factory leaves.
- **`machine_telemetry_spans.tenant_code` is `NOT NULL` with no default**, and
  the table is in `tenancy.SCOPED_MODELS`. Its `machine_id` foreign key onto
