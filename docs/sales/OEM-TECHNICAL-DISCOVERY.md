...
| `SHARE_ALARMS` grant | The grant key exists in `oem_sharing.ALL_GRANTS` and a factory can tick it ("Equipment alarm codes raised by this machine"), but **no code reads it**. The same is true today of `SHARE_TELEMETRY`, `SHARE_SERVICE_STATUS`, `SHARE_MAINTENANCE_HISTORY` and `SHARE_DOWNTIME`. Only `SHARE_MACHINE_HEALTH` and `SHARE_OPERATING_HOURS` actually gate a field (`oem_sharing.fleet_row`). Never demo a ticked box as a working feed. |
