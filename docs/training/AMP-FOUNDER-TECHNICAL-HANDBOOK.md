...
2. **LLM-backed — exactly ONE feature, off by default, and no hosted key is required.** `backend/ai_copilot.py` (wrapped by `ai/copilot.py`) is the *only* code that calls a Large Language Model, through one provider interface with three implementations: Anthropic (`ANTHROPIC_API_KEY`), Gemini (`GEMINI_API_KEY`, demo-only) and a **self-hosted, open-weight model** (`AMP_LLM_BASE_URL` + `AMP_LLM_MODEL`, any OpenAI-compatible runtime — Ollama, llama.cpp, vLLM; ADR-0023). A hosted key is optional: AMP runs the whole Copilot on the self-hosted model alone (pinned in `test_copilot_local_provider.py` §4b). The model is a **language interface, tool selector and evidence explainer**, never the source of a fact: it chooses typed AMP tools, AMP authorizes and runs them, the model words the evidence, and the **grounding gate** refuses any wording whose numbers, names, citations or links are not in that evidence. A self-hosted model's wording is used only once that exact model has **passed the Copilot evaluation** (`ai/adopted_models.json`, ADR-0023/0034); until then, and on any failure, AMP answers from its own engine (`"source":"rules"`). **A hosted key connects the platform; it does not speak for a company** (ADR-0037): a company's question, AMP's draft and the evidence behind it reach Anthropic or Gemini only after that company's own Admin turns on `external_model` under **Agent Activity → AI consent** — the same stored, audited, revocable decision as learning consent, read on every request at the one place the Copilot builds a request's model (`ai_copilot._copilot_llm`). Without it, and for a founder previewing the company, `/ai/ask` and `/ai/report` answer from AMP's own engine and the `note` says why; `/ai/status` reports it under `external`. **The consent names the provider it was given for** (ADR-0038, migration `0012_consent_scope`): the row records the hosted provider configured when the Admin said yes, and the gate honours it for that provider only — switching providers, or auto-detection falling through to the other key, lapses it until the Admin decides again, and a grant with no hosted provider configured is refused (nothing to consent to). Pinned by `test_external_model_consent.py` and `mutate_external_model_consent.py` (23/23), the migration by `test_migration_0012_consent_scope.py` and `verify_pg_consent_scope.py`. Surfaces at 3 endpoints: `/ai/status`, `/ai/ask`, `/ai/report` — and since ADR-0034 the report goes through the same tools and gate as every other answer.
...
**The pending item is held (ADR-0015 addendum, 2026-09-17).** A fifth check runs last: (5) the item the decision would move still exists in the action's tenant and is still pending (task `Proposed` / PO `Draft` / escalation `Proposed`) → 409 — so a decision is never recorded against an item that did not move. The decide route then **withdraws** that proposal (`Cancelled`, `decided_by=system-withdrawn`) with a compare-and-set, so a double-click can never overwrite the first decision (the loser gets 400 `Already approved`). Two decisions at the *same instant* are kept apart by `SELECT … FOR UPDATE` on the item row — which only PostgreSQL can hold, so that race (`test_agent_item_lock.py` section 11) SKIPs on SQLite and is proven by `verify_pg_approvals.py` on a scratch PostgreSQL (locally, and in CI's PostgreSQL migration job), together with the mutations that remove the lock. Check 3 now applies to **approve only**: rejecting an expired proposal is the recorded way to release its item, and approving one answers 409 "can only be rejected, which releases the item it holds" (each `/agent-actions` row carries `expired`; the inbox shows the same sentence and greys out Approve). Only an agent puts an item *into* its pending status: a PATCH or POST that sets `Proposed`/`Draft` by hand is 400, because re-arming an orphaned proposal would let an approval record someone else's edits as the agent's. `GET /agent-actions` pages (`limit` ≤ 300, `offset`) and the inbox loads older proposals on request — the list is the only way out for a held item. The other half is the **lock**: while a proposal is undecided, `approvals.refuse_if_awaiting_decision` refuses every PATCH and DELETE on its task/PO/escalation with 409, for every role including Admin (a Draft PO cannot have stock received against it). An item is *held* only when it is in its pending status **and** a `Proposed` AgentAction points at it (and the item was written no later than that action — SQLite reuses a deleted row's id, and a newer row with the old id is not the proposal's) **and** the tenant's plan includes the decision API (Intelligence Pack) — a human-created "Proposed" task is an ordinary row. On a plan *without* the Intelligence Pack the item stays editable, but the edit or delete **withdraws** the proposal in the same transaction: measured before, an Operator rewrote a growth tenant's proposed task, the founder upgraded the plan, and a Supervisor's approval recorded the agent's proposal over the Operator's content. Every withdrawal writes an `AuditLog` row (`withdraw_agent_action`) naming the approver, the editor, or `system` for the (unwired) orphan sweep. The lists carry `awaiting_approval {agent_action_id, agent, expired}` (a PATCH response's is null by construction: a PATCH only succeeds on a row that is not held); CMMS, Purchasing and Escalations read only that flag to grey out controls, hide Delete and show who decides. Every screen that decides a proposal (the Approvals inbox, Mission Control, the agent drawer and the machine cockpit drawer) greys out Approve on an expired one and reloads after a refused decision.
...
### Common confusion
- **"AMP uses machine learning to predict failures."** Not on any existing screen — Machine Health, the briefing and the agents use the **deterministic risk score** (hand-tuned thresholds). Say "rule-based predictive scoring" — and since ADR-0027 the Machine Health card shows that scoring's arithmetic: every rule it ran, the points each took off, the value it read and the threshold it read against, with the note that none of it is machine learning — and, since 2026-09-21, whether the score read anything at all: a machine with no downtime row, no production record and no breakdown transition in the 30-day risk window used to read "all 11 health rules passed" (100, the healthiest in the fleet) because every history rule read a 0 that was never recorded; now the engine's row carries `has_recorded_input`, the explanation is PARTIAL DATA with those six rules "not measured" and a note that the score is an absence, not a clean bill, the twin carries `health_measured`, and the Factory Pulse / Machine Health average is over the measured machines only (null, never 0, when there are none). AMP also ships an AMP-native failure-risk model (below), shown on its own card beside the rule and evaluated on **synthetic** machines only. Never quote its numbers as accuracy on a real plant.
...
- **"The copilot needs an API key."** No. The *rule-based* assistant (which powers most of the chat experience) needs nothing; the *LLM* copilot needs either a hosted key **or** a self-hosted OpenAI-compatible endpoint (`AMP_LLM_BASE_URL` + `AMP_LLM_MODEL`, ADR-0023/0034). AMP is pinned to run with no hosted key at all (`test_copilot_local_provider.py` §4b). No self-hosted model is currently promoted: `qwen3:8b` passed the first evaluation set and did not re-earn the gate when follow-up cases were added (AMP-NATIVE-MODEL-ACCEPTANCE.md §10), so the rules answer everywhere.
- **"The copilot remembers the conversation."** It does not — and that is the design (ADR-0035). The *screen* sends its own prior turns with each question (`thread`: each earlier question and the tool calls AMP ran for it, never an answer or its evidence), so "and its downtime?" can be resolved to the machine the conversation named. The server stores nothing, the referent is looked up through the caller's own scoped machine list, and every tool the follow-up runs is authorized afresh for the current role, tenant and licence. A forged thread naming another company's machine resolves to nothing (`test_copilot_follow_ups.py`, the evaluation's `ADVERSARIAL_THREADS`).
...
|---|---|---|---|---|
| `failure_risk` | Chance each machine STARTS a breakdown in the next 7 days | Synthetic fleet (`failure_risk/synthetic.py`, seed 20260917), Tuesday 10:00 snapshots only | Rule scorer: PR-AUC 0.094 vs model 0.194 (paired +0.099, 95% [0.029, 0.175]); ROC-AUC 0.612 vs 0.698. **But** on this data the rule barely works (2 of its 11 components fire on every row, 5 never fire), and one raw input, `reject_rate_drift`, ties the model (model minus it +0.023, [−0.034, +0.078]) | **Adopted, on synthetic data.** Since ADR-0027 it finally has a screen: per-machine estimates under its own model card (Admin/Supervisor) and the role-restricted `get_failure_risk` Copilot tool. Every estimate is labelled MODEL ESTIMATE, the result state is always MODEL NOT VALIDATED, the synthetic-only caveat rides on every row, the rule score sits beside it, and an artifact that does not verify produces no number at all. Replaces nothing. The adoption shows it beat a weak rule, not that it adds anything over one input. |
...
**The Copilot can PROPOSE an action, and still writes nothing** (ADR-0039). Ask it to "raise a maintenance task on CNC-01" and it answers with a **draft**: the task AMP would propose, with the evidence behind it, and a card saying plainly that nothing has been created. Three separate human-authenticated steps stand between that draft and anything happening, and a language model is at none of them. `ai/tools/actions.py` only READS — registering a tool that writes would put a model at the point a row comes into existence, which is exactly what the chain below forbids. A person raises the draft through `POST /agent-actions/propose`, which accepts only `kind` and `machine_id` and re-derives the priority, the task type and the wording from the machine itself, so the client's copy of the draft can never become what is written. That creates a `MaintenanceTask` "Proposed" plus an `AgentAction` "Proposed" — the identical path the five agents use — and it takes effect only when a person approves it through `approvals.authorise` (ADR-0005), which freezes the outcome baseline (ADR-0029). `agent="copilot"` can **never** auto-approve, whatever `AUTO_APPROVE_AGENTS` or a tenant policy says: a request that arrived as conversational text is the one kind that must always be decided by a human. **The Risk Radar offers the same thing.** Every risk carries an `action` — a sentence naming the next step, because the card used to end in a deep link and a list of eight things about to go wrong with no remedy reads as an alarm panel, not an advisor. A machine likely to stop also carries `propose`, the same draft, so an owner can raise it from the radar. Most risks carry no proposal and that is deliberate: ordering stock and chasing a customer are not AMP's to do, and `ai/risk_radar._risk()` raises on any kind outside `ev.PROPOSABLE_KINDS`. Both surfaces write through ONE `frontend/components/ProposeActionButton.tsx`, so the request exists in a single place. Where to change it: the drafts in `ai/tools/actions.py`, the kinds AMP can carry out in `ev.PROPOSABLE_KINDS` (and its mirror in `frontend/lib/evidence.ts`), the command phrases in `orchestrator._ACTION_PHRASES`, the write in `agent_routes.propose_agent_action`, the cards in `CopilotProposal.tsx` and `RiskRadarSection.tsx`. Pinned by `test_copilot_actions.py` (14 sections) and `mutate_copilot_actions.py`.
...
**Inference vs learning.** Scoring a tenant's records with a synthetic-trained model learns nothing (tested: the artifact and loaded model are unchanged after scoring, and tenant B's result does not depend on tenant A). Fitting anything to a tenant's own data is learning. The only such capability, `telemetry_baseline`, runs only after that tenant's Admin turns it on under **Agent Activity → AI consent**. The decision is stored in `ai_learning_consents` (migration `0009_native_ai_consent`), committed in the SAME transaction as its audit row (if the audit fails, nothing changes), checked on every request with no cache, and revocable. No row means no. Nothing learned is stored. The same card and table hold the second decision, `external_model` (ADR-0037): not learning, but the company's data leaving AMP for a hosted language model — the same rules, enforced where the Copilot builds its model rather than here.
...
- A full build INTO THE SHIPPED FOLDER re-runs the committed test set; the ledger it reads there then records a second run and the gate fails as `test_set_reused`. That is an audit trail, not a lock: the ledger only counts runs someone commits. A build into another folder, a `build()` call in Python, or the CI reproduce suites all re-score the test set without recording it. A new verdict needs a fresh test set (a new seed or question set) and the new hash pinned in code.
- `python -m amp_ai.failure_risk.diagnose` reproduces the post-hoc failure-risk diagnostics (the rule's per-component firing rates, the single-input baseline, the Tuesday 10:00 scope) that the model card and ADR-0020 quote.
...
**Say it like this:** "AMP has its own small models that run inside AMP. We built and tested them on simulated plants, next to our existing rules, and each model card shows what they did and did not show, including their weaknesses. None of them replaces a screen you use today, and none of them learns from your data unless your Admin turns that on, which you can turn off at any time."
...
### Quick recap
AMP's "AI" = mostly **deterministic rules** + read-models, **one optional LLM** feature (off until a hosted key or a self-hosted model is configured, always with a rules fallback; a hosted key is used for a company only once that company's Admin consents, ADR-0037; `qwen3:8b` passed the first evaluation set and did not re-earn the gate on the enlarged one, so no self-hosted model is currently promoted and every answer — production included — comes from the rules), and **three small AMP-native models** trained or evaluated on synthetic data only: one adopted (failure risk, beside the rule on its own view) and two not (ADR-0020). Five agents observe events and **propose** actions into a pending state, logged as `AgentAction` rows that are simultaneously audit trail and approval queue; the `approvals.py` gate (re-checking the DB, not the token) makes a human the authority. Only Reorder auto-approves, and only into a Draft.
...
|---|---|---|---|
| **Operator** | all factory reads (tenant-scoped) | shop-floor writes: production records, downtime logs, quality checks, WO progress, raising a material issue slip, acknowledging recommendations and notifications | admin writes; changing a machine's status or entering a shift (Supervisor+: `PATCH /machines/{id}/status`, `POST /shifts`); **approve agent actions**; **edit an agent proposal still awaiting approval** (nobody can — see below); remnants, slip approval/issue, GRNs, cycle counts (Supervisor+); deletes/voids; user/BOM/config admin. The screens offer an Operator none of these (lib/roles.ts) — a control the server would 403 is not shown, and a note says who can |
| **Supervisor** | all reads | + management writes: machine status, shifts, costs, GRN/inventory receipts, remnants, slips, cycle counts, reports, recommendations, machine metadata; **approve/reject agent actions** | Admin-only: deletes, voids, cycle-count approval, machine create/delete, CSV imports, `/users`, SaaS registry, BOM writes, **read audit logs** |
...
| **Audit log** | best-effort `AuditLog` rows; `actor` stamped from the JWT, not the body (no forged provenance); Admin-only to read. Every `/users` write is audited — create, delete, **role change (old → new role)** and **admin password reset (whose, never the password)**; the last two were silent until 2026-09, and `test_users_routes` now fails for any new users write handler that does not call `log_audit`. Every **GMATS inventory** write is audited too (all 13: item create/edit/correct/delete, stock-in, alias, proforma create/cancel, invoice generate/void, MIN issue/void, CSV import). Each row names the record (item code, PI/INV/MIN number) and the change (old → new stock, quantities restored), and is filed under the **company whose records changed**, so a founder's correction made from DEFAULT shows in the customer's log. Before 2026-09 none were, although the GMATS pilot proposal promised a "full audit trail". A voided invoice or MIN is deleted, so its audit row is the only record it existed. `test_gmats_writes_leave_a_trail` finds every write route by AST and fails until a new one audits itself after its last commit. "Best-effort" is literal: `log_audit` commits separately and swallows its own failure, so an audit write that fails does not undo the change it describes | `platform_routes.log_audit` |
...
| **Held agent proposals** | an undecided agent-proposed task / PO / escalation refuses every edit and delete (409, any role) until an Admin/Supervisor approves or rejects it; no decision is recorded unless the item moves with it; nobody sets an item's pending status by hand (400); on a plan without Approvals an edit withdraws the proposal instead, so an upgrade cannot re-arm it; every withdrawal is audit-logged against who caused it (ADR-0015 addendum) | `approvals.refuse_if_awaiting_decision`, `approvals._check_item`, `approvals.refuse_manual_pending_status`, `approvals.withdraw` |
...
| `ANTHROPIC_API_KEY` / `GEMINI_API_KEY` | a hosted language model for the copilot (optional; neither is required). Used for a company only after that company's Admin consents under AI consent (ADR-0037) |
| `AMP_LLM_BASE_URL` + `AMP_LLM_MODEL` | a self-hosted model behind any OpenAI-compatible runtime, e.g. `http://127.0.0.1:11434/v1` + `qwen3:8b` (optional; its wording is used only once that exact model has passed the Copilot evaluation, ADR-0023/0034) |
| `AMP_LLM_PLAN_TOKENS` / `AMP_LLM_TIMEOUT` | room a reasoning model gets to name its tools (default 1200, measured) and seconds per call (default 30) |
...
| LLM copilot | 🟡 **BUILT · no model currently promoted · off on production** | ADR-0023/0034/0035: runs on a hosted key **or** a self-hosted model (`AMP_LLM_BASE_URL`), no hosted key required; `qwen3:8b` passed the 135-question set with zero disclosures and did not re-earn the gate on the 150-question set that added follow-ups (one core case, three times; still zero disclosures) — not promoted; production has no GPU, so it answers from AMP's own engine; every model sentence passes the grounding gate and the rules answer without any model |
...
*"AMP is a working, multi-tenant, event-driven manufacturing platform: real MES, real tenant isolation, real live telemetry ingest (MQTT/HTTP + WebSocket), real AI agents under human approval, and a real OEM fleet/consent platform — all CI-tested and deployed with migrations-gated releases. The intelligence is deterministic rules on every screen, plus a grounded copilot that can run on a self-hosted model (measured on a 339-answer evaluation with zero disclosures; no model currently passes the routing bar, so the rules answer) and three small AMP-native models trained or evaluated on synthetic data only — one adopted beside the rule, none proven on real failures. Direct PLC-protocol connectivity is simulated behind a ready adapter interface, pending per-OEM edge agents. Everything a customer clicks is real; the machine-side drivers and ML validated on a real plant are the honest next frontier."*

### Quick recap
The platform, MES, isolation, events, agents, OEM, SaaS, and CI/CD are **real and working**. The **data source** (simulator), **direct PLC protocols** (adapter framework, no drivers), **LLM** (optional; no self-hosted model currently passes the gate, the rules answer), and **ML** (three small synthetic-trained models, one adopted, none validated on a real plant) are the clearly-labelled edges. Never claim ML validated on real data, or live PLC connectivity, that you don't have — and never market one kind of engine as another (the table below says which is which).
...
| Copilot planning and wording (`ai/orchestrator.py`, `ai/llm.py`) | **LLM-ASSISTED** | A language model names which authorized tools to run and words the evidence they return; AMP decides authorization, runs the tools and grounds every sentence (ADR-0022/0023) | No self-hosted model currently passes the gate on the current question set. `qwen3:8b` (Apache 2.0) passed the 135-question set on 2026-09-20 — 279/279 grounded, 0 disclosures (ADR-0034) — and on the 150-question set that added follow-ups (ADR-0035) missed one core case three times (`is it running now?` about a named machine → the plant-wide status list), so it was **not re-promoted**; 0 disclosures across 378 adversarial prompts in those two runs (AMP-NATIVE-MODEL-ACCEPTANCE.md §10). The committed record is the failing one. Production has no GPU, so production answers from the rules — as does everywhere else until a model passes; a model never invents a figure because the gate refuses one |
