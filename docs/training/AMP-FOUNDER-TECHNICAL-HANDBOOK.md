...
| **Held agent proposals** | an undecided agent-proposed task / PO / escalation refuses every edit and delete (409, any role) until an Admin/Supervisor approves or rejects it; no decision is recorded unless the item moves with it (ADR-0015 addendum) | `approvals.refuse_if_awaiting_decision`, `approvals._check_item` |
