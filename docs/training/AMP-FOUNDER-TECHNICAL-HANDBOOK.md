...
|---|---|---|---|---|
| `failure_risk` | Chance each machine STARTS a breakdown in the next 7 days | Synthetic fleet (`failure_risk/synthetic.py`, seed 20260917), Tuesday 10:00 snapshots only | Rule scorer: PR-AUC 0.094 vs model 0.194 (paired +0.099, 95% [0.029, 0.175]); ROC-AUC 0.612 vs 0.698. **But** on this data the rule barely works (2 of its 11 components fire on every row, 5 never fire), and one raw input, `reject_rate_drift`, ties the model (model minus it +0.023, [−0.034, +0.078]) | **Adopted, on synthetic data.** Shown only at `GET /ai/native/failure-risk`, with the rule score beside it. Replaces nothing. The adoption shows it beat a weak rule, not that it adds anything over one input. |
