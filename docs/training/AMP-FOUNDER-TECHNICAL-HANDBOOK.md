...
- A new stock document (like GRN) → `enterprise_inventory_routes.py` + `doc_numbers.allocate`. **Never `count() + 1`.** A count is a population, not a sequence: delete one row and the next document gets a number still printed on a live one. That is how GMATS issued two live tax invoices numbered INV-7002 after a void, until 2026-09 (ADR-0012 addendum). `test_gmats_document_numbers_never_reused` fails CI for any route module that builds a number from a count.
