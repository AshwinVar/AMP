...
export default function AIInsightsSection({ recommendations, insights, generateRecommendations, updateRecommendation, onOpen }: {
  ...
  return (
    <section className="mt-8 space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">{recommendations.map(row=><div key={row.id} className="rounded-2xl bg-slate-900 border border-slate-800 p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-slate-400">{row.recommendation_type}</p><h3 className="text-xl font-bold mt-1">{row.title}</h3></div><span className={`rounded-full px-3 py-1 text-xs border ${severityStyle(row.severity)}`}>{row.severity}</span></div><p className="text-slate-300 mt-4">{row.message}</p>{row.action && (<p className="text-sm text-slate-200 mt-3"><span className="text-[10px] uppercase tracking-wide text-indigo-300/80 mr-2">Do</span>{row.action}</p>)}{row.propose && (<ProposeActionButton kind={row.propose.kind} machineId={row.propose.machine_id} label="Propose a maintenance task" onOpen={onOpen} compact />)}<div className="mt-4 flex items-center justify-between"><p className="text-sm text-slate-400">Confidence: {row.confidence}%</p><select className="bg-slate-950 border border-slate-700 rounded-lg px-3 py-2" value={row.status} onChange={(e)=>updateRecommendation(row.id,e.target.value)}><option>Open</option><option>Acknowledged</option><option>Closed</option></select></div></div>)}</div>
    </section>
  );
}
