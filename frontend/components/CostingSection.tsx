...
export default function CostingSection({ costs, analytics, form, setForm, createCost, updateCost, deleteCost }: {
  ...
  return <section className="mt-8 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-4"><Kpi title="Cost Records" value={analytics?.total_cost_records ?? 0}/><Kpi title="Manual Cost" value={money(analytics?.manual_cost_total ?? 0)}/><Kpi title="Good Units" value={analytics?.production_units ?? 0}/><Kpi title="Cost / Unit" value={analytics?.cost_per_good_unit != null ? `${CURRENCY}${analytics.cost_per_good_unit.toFixed(2)}` : "—"}/><Kpi title="Receipt Units" value={analytics?.supplier_receipt_units ?? 0}/></div>
    ...
    <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5 overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead className="text-slate-400 border-b border-slate-800"><tr>{["Cost No","Type","Description","Amount","Department","Reference","Actions"].map(h=><th key={h} className="py-3 px-4">{h}</th>)}</tr></thead><tbody>{costs.map(row=><tr key={row.id} className="border-b border-slate-800"><td className="py-3 px-4 font-semibold">{row.cost_no}</td><td className="py-3 px-4">{row.cost_type}</td><td className="py-3 px-4">{row.description}</td><td className="py-3 px-4"><LiveInput className="w-24 bg-slate-950 border border-slate-700 rounded-lg px-2 py-1" type="number" value={row.amount} onCommit={(raw) =>updateCost(row.id,Number(raw))}/></td><td className="py-3 px-4">{row.department ?? "-"}</td><td className="py-3 px-4 text-slate-400" title="Recorded on the cost as data; the costing figures group by type and department, not by reference">{row.reference_type ? [row.reference_type, row.reference_id ? "#" + row.reference_id : ""].join(" ").trim() : "—"}</td><td className="py-3 px-4"><button onClick={()=>deleteCost?.(row.id)} className="text-red-400 border border-red-500/40 rounded-lg px-3 py-1">Delete</button></td></tr>)}</tbody></table></div>
  </section>;
}
