...
export default function CostingSection({ costs, analytics, form, setForm, createCost, updateCost, deleteCost }: {
  ...
  return <section className="mt-8 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-4"><Kpi title="Cost Records" value={analytics?.total_cost_records ?? 0}/><Kpi title="Manual Cost" value={`£${analytics?.manual_cost_total ?? 0}`}/><Kpi title="Good Units" value={analytics?.production_units ?? 0}/><Kpi title="Cost / Unit" value={`£${analytics?.cost_per_good_unit ?? 0}`}/><Kpi title="Receipt Units" value={analytics?.supplier_receipt_units ?? 0}/></div>
