...
type ChaseOrder = {
  order_no: string; product: string; due_date: string | null;
  order_quantity: number; dispatched_quantity: number;
  state: "at_risk" | "late"; days_to_due: number | null;
};

type RecentOrder = {
  order_no: string; product: string;
  order_quantity: number; dispatched_quantity: number;
  due_date: string | null; state: State;
};
...
// A right-hand drawer that drills into one customer: its unit fulfillment and
// delivery reliability, the order state mix, a due timeline, the orders to
// chase, and its recent orders. Self-fetches from /delivery-customer; closes on
// Escape.
export default function CustomerDrawer({ customer, onClose }: { customer: string; onClose: () => void }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setDetail(await apiGet<CustomerDetail>(`/delivery-customer?customer=${encodeURIComponent(customer)}`));
    } catch (e) {
  ...
  const upcomingPeak = detail ? Math.max(...detail.upcoming.map((u) => u.orders), 1) : 1;
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Delivery reliability</p>
          </div>
        ...
        {loading && !detail ? (
          <p className="text-slate-400 mt-6">Loading customer detail…</p>
        ) : detail ? (
          detail.total === 0 ? (
            <p className="text-slate-500 text-sm mt-6">No orders on record for {customer}.</p>
          ) : (
            <div className="mt-5 space-y-6">
              {/* Headline rates */}
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5">
                  <p className={`text-3xl font-bold ${rateColor(detail.fulfillment_rate)}`}>{detail.fulfillment_rate}%</p>
                  <p className="text-xs text-slate-500 mt-1">
                    units fulfilled · {detail.dispatched_units}/{detail.ordered_units}
                  </p>
                </div>
                <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5">
              ...
              <div className="grid grid-cols-4 gap-2">
                {(["delivered", "on_track", "at_risk", "late"] as State[]).map((k) => (
              ...
              {detail.overdue_units > 0 && (
                <p className="text-sm text-red-300">
                  {detail.overdue_units} unit{detail.overdue_units !== 1 ? "s" : ""} still owed on overdue orders.
                </p>
              )}

              {/* Due timeline */}
              {detail.upcoming.some((u) => u.orders > 0) && (
                <div>
                  ...
                  <div className="flex items-end gap-1 h-10">
                    {detail.upcoming.map((u) => (
                      <div
                        ...
                        style={{ height: `${u.orders === 0 ? 0 : Math.max(4, Math.round((u.orders / upcomingPeak) * 100))}%` }}
              ...
              {/* Orders to chase */}
              <div>
                <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">Orders to chase</h3>
                {detail.chase.length === 0 ? (
                  <p className="text-emerald-400 text-sm mt-3">Nothing at risk — this customer is on track.</p>
                ) : (
                    ...
                    {detail.chase.map((o) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm text-slate-200 truncate">{o.order_no}</p>
              ...
              <div>
                <h3 className="text-sm font-semibold text-slate-300 uppercase tracking-wide">
                  Recent orders · {detail.total}
                </h3>
                <ol className="mt-3 space-y-3">
                  {detail.recent.map((o) => (
                    <li key={o.order_no} className="border-b border-slate-800/70 pb-3">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium truncate">
                          {o.order_no} <span className="text-slate-500">· {o.product}</span>
                        </p>
                        <span className={`text-xs shrink-0 ${STATE_CLS[o.state]}`}>{STATE_LABEL[o.state]}</span>
                      </div>
                      <p className="text-xs text-slate-600 mt-0.5">
                        {o.dispatched_quantity}/{o.order_quantity} shipped
