  ...
  return (
    <section className="mt-8 space-y-6">
      ...
      <form onSubmit={createTask} className="rounded-2xl bg-slate-900 border border-slate-800 p-5 grid grid-cols-1 md:grid-cols-4 xl:grid-cols-8 gap-4">
        ...
        <select className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3" value={form.task_type} onChange={(e)=>setForm({...form,task_type:e.target.value})}>{statusOptions("MaintenanceTask","task_type",form.task_type).map(o=><option key={o}>{o}</option>)}</select>
        <select className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3" value={form.priority} onChange={(e)=>setForm({...form,priority:e.target.value})}>{statusOptions("MaintenanceTask","priority",form.priority).map(o=><option key={o}>{o}</option>)}</select>
        ...
        <select className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3" value={form.status} onChange={(e)=>setForm({...form,status:e.target.value})}>{statusOptions("MaintenanceTask","status",form.status).map(o=><option key={o}>{o}</option>)}</select>
        <button type="submit" className="rounded-xl bg-white text-slate-950 font-semibold px-4 py-3">Add Task</button>
      </form>
      <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5 overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="text-slate-400 border-b border-slate-800"><tr>{["Task","Machine","Type","Priority","Assigned","Planned","Downtime","Status","Actions"].map(h=><th key={h} className="py-3 px-4">{h}</th>)}</tr></thead><tbody>{tasks.map(row=><tr key={row.id} className="border-b border-slate-800"><td className="py-3 px-4 font-semibold">{row.task_no}</td><td className="py-3 px-4">{getMachineName(row.machine_id)}</td><td className="py-3 px-4">{row.task_type}</td><td className="py-3 px-4">{row.priority}</td><td className="py-3 px-4">{row.assigned_to}</td><td className="py-3 px-4">{row.planned_date}</td><td className="py-3 px-4"><LiveInput className="w-20 bg-slate-950 border border-slate-700 rounded-lg px-2 py-1" type="number" value={row.downtime_minutes} disabled={isHeld(row)} onCommit={(raw) =>updateTask(row.id,row.status,Number(raw))}/></td><td className="py-3 px-4"><select className="bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 disabled:opacity-60" aria-label={`Status of ${row.task_no}`} value={row.status} disabled={isHeld(row)} onChange={(e)=>updateTask(row.id,e.target.value,row.downtime_minutes)}>{statusOptions("MaintenanceTask","status",row.status).map(o=><option key={o}>{o}</option>)}</select><AwaitingApprovalNotice hold={row.awaiting_approval}/></td><td className="py-3 px-4">{deleteTask && !isHeld(row) && <button onClick={()=>deleteTask(row.id)} className="text-red-400 border border-red-500/40 rounded-lg px-3 py-1">Delete</button>}</td></tr>)}</tbody></table></div>
    </section>
  );
}
