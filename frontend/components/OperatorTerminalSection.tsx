...
// What an operator can set. Deliberately NOT the full set of words stored in the
// column: the simulator and e2e_sim also write "In Progress", which is live work
// (ai/workforce.py counts it as such) but not a state anyone picks by hand.
const JOB_STATUSES = ["Started", "Paused", "Completed"];
...
export default function OperatorTerminalSection({ machines, workOrders, productionPlans, executions, analytics, form, setForm, createExecution, updateExecution, deleteExecution, getMachineName }: {
  ...
  return <section className="mt-8 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-7 gap-4"><Kpi title="Jobs" value={analytics?.total_jobs ?? 0}/><Kpi title="Started" value={analytics?.started ?? 0}/><Kpi title="Paused" value={analytics?.paused ?? 0}/><Kpi title="Completed" value={analytics?.completed ?? 0}/><Kpi title="Other" value={analytics?.other ?? 0}/><Kpi title="Good" value={analytics?.good_count ?? 0}/><Kpi title="Quality" value={analytics?.quality_rate == null ? "—" : analytics.quality_rate + "%"}/></div>
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">{executions.map(row=><div key={row.id} className="rounded-2xl bg-slate-900 border border-slate-800 p-5"><div className="flex items-start justify-between"><div><p className="text-sm text-slate-400">{row.execution_no}</p><h3 className="text-xl font-bold">{getMachineName(row.machine_id)}</h3><p className="text-slate-400">Operator: {row.operator_name}</p></div><select className="bg-slate-950 border border-slate-700 rounded-lg px-3 py-2" value={row.job_status} onChange={(e)=>updateExecution(row.id,e.target.value,row.good_count,row.rejected_count)}>{/* The stored value first when it is not one of the three an operator can pick. The simulator and e2e_sim write "In Progress", and a select whose value matches no option renders blank — so the row showed no status at all, and the next edit silently overwrote it. */}{row.job_status && !JOB_STATUSES.includes(row.job_status) && <option>{row.job_status}</option>}{JOB_STATUSES.map((s)=><option key={s}>{s}</option>)}</select></div><div className="mt-4 grid grid-cols-2 gap-3"><LiveInput className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3" type="number" value={row.good_count} onCommit={(raw) =>updateExecution(row.id,row.job_status,Number(raw),row.rejected_count)}/><LiveInput className="bg-slate-950 border border-slate-700 rounded-xl px-4 py-3" type="number" value={row.rejected_count} onCommit={(raw) =>updateExecution(row.id,row.job_status,row.good_count,Number(raw))}/></div><button onClick={()=>deleteExecution?.(row.id)} className="mt-4 text-red-400 border border-red-500/40 rounded-lg px-3 py-1">Delete</button></div>)}</div>
  </section>;
}
