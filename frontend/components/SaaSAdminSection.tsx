...
export default function SaaSAdminSection({ tenants, analytics, form, setForm, createTenant, updateTenant, deleteTenant, provisionAdmin, adminCreds, clearAdminCreds }: {
  ...
  return <section className="mt-8 space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 xl:grid-cols-7 gap-4"><Kpi title="Tenants" value={analytics?.total_tenants ?? 0}/><Kpi title="Trial" value={analytics?.trial ?? 0}/><Kpi title="Active" value={analytics?.active ?? 0}/><Kpi title="Past Due" value={analytics?.past_due ?? 0}/><Kpi title="Cancelled" value={analytics?.cancelled ?? 0}/><Kpi title="MRR" value={money(analytics?.monthly_recurring_revenue ?? 0)}/><Kpi title="Seats" value={analytics?.total_seats ?? 0}/></div>
    ...
    <div className="rounded-2xl bg-slate-900 border border-slate-800 p-5 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="text-slate-400 border-b border-slate-800"><tr>{["Code","Company","Industry","Plan","Status","Seats","Fee","Actions"].map(h=><th key={h} className="py-3 px-4">{h}</th>)}</tr></thead><tbody>{tenants.map(row=><tr key={row.id} className="border-b border-slate-800"><td className="py-3 px-4 font-semibold">{row.company_code}</td><td className="py-3 px-4">{row.company_name}</td><td className="py-3 px-4">{row.industry ?? "-"}</td><td className="py-3 px-4">{row.plan_name}</td><td className="py-3 px-4"><select className="bg-slate-950 border border-slate-700 rounded-lg px-2 py-1" value={row.subscription_status} onChange={(e)=>updateTenant(row.id,e.target.value)}><option>Trial</option><option>Active</option><option>Past Due</option><option>Cancelled</option></select>{row.subscription_status === "Trial" && row.trial_days_left != null && <div className={`text-xs mt-1 ${row.trial_days_left <= 5 ? "text-amber-400" : "text-slate-400"}`}>{row.trial_days_left > 0 ? `${row.trial_days_left}d left` : "expired — logins blocked"}</div>}</td><td className="py-3 px-4">{row.seats}</td><td className="py-3 px-4">{money(row.monthly_fee)}</td><td className="py-3 px-4 whitespace-nowrap"><button onClick={()=>provisionAdmin?.(row.id)} className="text-emerald-300 border border-emerald-500/40 rounded-lg px-3 py-1 mr-2">Create admin</button><button onClick={()=>deleteTenant?.(row.id)} className="text-red-400 border border-red-500/40 rounded-lg px-3 py-1">Delete</button></td></tr>)}</tbody></table></div>
  </section>;
}
